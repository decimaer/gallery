...
import User from '../models/userModel-mongoose';
import { middlewareType } from '../types/expressTypes';

import * as authController from './authController-OLD';
...
export const createUser: middlewareType = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json(await authController.loggedInUserResponse(newUser._id.toString()));
  } catch (error) {
    res.status(400).json({
      status: 'fail',
      message: error,
